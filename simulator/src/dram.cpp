...
void Dram::load_stats(Stats* stats) { this->stats = stats; }
...
void Dram::issue_request(int addr) {
...
hd_t Dram::get_mem_word(int addr, int dst_reg) {
  auto point = addr2rowcol(addr);

  if (active_row != point.first) {
    buffer2row(active_row);
